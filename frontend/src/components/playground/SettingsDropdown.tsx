...
export const SettingsDropdown = () => {
  ...
  return (
    <DropdownMenu.Root modal={false}>
      <DropdownMenu.Trigger className="group inline-flex max-h-12 items-center gap-1 rounded-md hover:bg-gray-200 bg-gray-100 border-gray-800 p-1 pr-2 text-gray-900 my-auto text-sm flex gap-1 pl-2 py-1 h-full items-center">
        Settings
        <ChevronIcon />
      </DropdownMenu.Trigger>
      <DropdownMenu.Portal>
        <DropdownMenu.Content
          className="z-50 flex w-60 flex-col gap-0 overflow-hidden rounded text-gray-900 border border-gray-800 bg-gray-100 py-2 text-sm"
          sideOffset={5}
          collisionPadding={16}
        >
          {settingsDropdown.map((setting) => {
            ...
            return (
              <DropdownMenu.Label
                ...
                className="flex max-w-full flex-row items-end gap-2 px-3 py-2 text-xs hover:bg-gray-200 cursor-pointer"
              >
